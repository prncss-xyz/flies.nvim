...
describe('bigword', function()
  it('should respect basic pattern', function()
    local t = M.bigword()
    assert.is.falsy(t:search_upward({ 1, 2 }, 1))
    with_fake_buf({ ' aaa.bbb cc ' }, function()
      assert.are.same(
        { { 1, 2 }, { 1, 2 }, { 1, 8 }, { 1, 9 } },
        { t:search_upward({ 1, 2 }, 1) }
      )
    end)
  end)
...
describe('variable segment', function()
  it('description', function()
    ...
    with_fake_buf({ ' bb_cc ' }, function()
      assert.are.same(
        { { 1, 2 }, { 1, 2 }, { 1, 3 }, { 1, 4 } },
        { t:post_proc(t:search_upward({ 1, 2 }, 1)) }
      )
      assert.are.same(
        { { 1, 4 }, { 1, 5 }, { 1, 6 }, { 1, 6 } },
        { t:post_proc(t:search_upward({ 1, 5 }, 1)) }
      )
    end)
  end)
end)
